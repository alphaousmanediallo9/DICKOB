...
const SeconnectePourAdmin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/seconnectePourAdminr', { email, password }); // Envoi des données de connexion au serveur
  ...
  return (
    <Container>
      ...
      >
        <h3>
          <Typewriter
            words={['Mes salutations', 'Vous êtes sur la', 'Page de Connexion', "Saisissez les bonnes", "Informations nécessaires", "S'il vous plaît"]}
            loop={1}
            cursor
            cursorStyle=" -°-"
            typeSpeed={70}
        ...
        >
          <a href='/admindasboardgenerale' c>
            <VscEyeClosed className=' text-white' size={18}  style={{ marginRight: '10px' }} />
            </a> Quitter
        </Button>
...
export default SeconnectePourAdmin;
